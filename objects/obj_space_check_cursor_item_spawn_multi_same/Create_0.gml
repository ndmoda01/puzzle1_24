...
if (place_meeting(x,y,obj_space_check_stop)){
	...
	//spawn the target
	var _target_id = instance_create_layer(_random_free_space_index[0],_random_free_space_index[1],"Instances_1",obj_target);
	
	//spawn the item and set it's target
	var _new_item = instance_create_layer(obj_item_spawner.x, obj_item_spawner.y,"Instances_1", obj_item_spawner_imposter);
		...
		else if (obj_game.game_type == BLOCKOUT) and (obj_item_spawner.player1_queued_items >0) _new_item.my_type = BLOCK;
