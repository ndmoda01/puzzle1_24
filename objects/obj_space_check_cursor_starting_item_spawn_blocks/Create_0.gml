...
var _num_of_starting_blocks = obj_game.level_starting_blocks;
	...
	case 2:
		if (obj_game.sbvs == true) and (obj_game.multiplayer_mode == MULTI_COMP){
			if (instance_exists(obj_space_check_start_block)){
				...
				if (_num_of_starting_blocks <=12) obj_space_check_start_block.y = 160;	
				if (_num_of_starting_blocks >12)  and (_num_of_starting_blocks <=24) obj_space_check_start_block.y = 144;	
				if (_num_of_starting_blocks >24) and (_num_of_starting_blocks <=36) obj_space_check_start_block.y = 128;	
			}
		}
		else{
			if (instance_exists(obj_space_check_start_block)){
...
start_x = obj_space_check_start_block.x;
start_y = obj_space_check_start_block.y;
x = start_x;
y = start_y;
